...
class SurfacesAnimation(ThreeDScene):
    def construct(self):
        axes = ThreeDAxes()
        baal = ParametricSurface(
            lambda u, v: np.array([
                3-2*u+v,
                u,
                v
                ]),
                v_min=-10,v_max=10,u_min=-10,u_max=10,checkerboard_colors=[RED_D, RED_E],
            resolution=(50, 50)).scale(0.2)
        saal = ParametricSurface(
            lambda u, v: np.array([
                1-u-v,
                u,
                v
                ]),
                v_min=-10,v_max=10,u_min=-10,u_max=10,checkerboard_colors=[BLUE_D, BLUE_E],
            resolution=(50, 50)).scale(0.2)
        ...
        #self.wait()
        self.play(FadeIn(baal))
        self.wait()
        self.play(FadeIn(saal))
        self.wait(10)
        return
